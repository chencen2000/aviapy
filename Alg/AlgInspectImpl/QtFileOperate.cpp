...
bool CQtFileOperate::IsPathExist(std::string strPath)
{
   QString str = QString::fromStdString(strPath);
    return IsPathExist(str);
}
bool CQtFileOperate::QTMessageBox(std::string strMessage)
{
   QMessageBox::about(NULL, "警告", QString::fromStdString(strMessage));
    return true;
}
...
bool CQtFileOperate::CreateMultiLevelPath(std::string strPath)
{
    QString str = QString::fromStdString(strPath);
     return CreateMultiLevelPath(str);
}
...
QString CQtFileOperate:: Decrypt(const QString &strInput)
{
 ...
 std::string & CQtFileOperate::std_string_format(std::string & _str, const char * _Format, ...)
{
    std::string tmp;
    va_list marker = NULL;
 ...
 std::string CQtFileOperate::GetTimeString()
 {
	 std::string tmp;
	 SYSTEMTIME st = { 0 };
	 GetLocalTime(&st);
	 std_string_format(tmp,"%d%02d%02d%02d%02d%02d", st.wYear,st.wMonth,st.wDay,st.wHour,st.wMinute,st.wSecond);
	 return tmp;
 }